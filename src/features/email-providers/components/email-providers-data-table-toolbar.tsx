...
import { emailProviderTypeValues } from "@/features/email-providers/types/email-provider.types";
...
interface EmailProvidersDataTableToolbarProps<TData> {
  table: Table<TData>;
}

const providerTypeFilterOptions = emailProviderTypeValues
  .map((value) => ({ label: getProviderTypeLabel(value), value }))
...
export function EmailProvidersDataTableToolbar<TData>({
  table,
}: EmailProvidersDataTableToolbarProps<TData>) {
  const {
    search,
    providerType,
    isActive,
  ...
  const hasFilters = Boolean(
    search || providerType || isActive || fromDate || toDate,
  );
