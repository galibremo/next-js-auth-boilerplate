...
} from "@/features/sessions/utils/session-format";
...
export function createSessionColumns({
	...
	return [
		...
		},
		{
			...
			cell: ({ row }) => (
				<span className="block max-w-56 break-all whitespace-normal">{row.original.userAgent}</span>
			)
		},
		{
			...
			header: "Login method",
			cell: ({ row }) => (
				<Badge variant="secondary">{formatLoginMethod(row.original.loginMethod)}</Badge>
			)
		},
		{
